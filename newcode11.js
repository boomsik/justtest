...
const main = async () => {
    ...
    posts.forEach((post) => {
        if (post.text.includes("Deployed from sTONks bot")) {
            ...
            const tokenBalanceMatch = post.text.match(/(\d+(\.\d+)?)💎/);
            ...
            const tokenDescription = tokenDescriptionMatch
                ...
                : "Описание не найдено";
            const tokenBalance = tokenBalanceMatch
                ? `${tokenBalanceMatch[1]} TON`
                : "Баланс не найден";

            const message = `<a href="https://tonscan.org/address/${tokenAddress}">${tokenAddress}</a>\nDescription: ${tokenDescription}\nBalance: ${tokenBalance}`;
            console.log(message);
            console.log("=========================");
