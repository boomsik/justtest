...
const sendMessageToChannel = async (client, channelUsername, message) => {
    const channel = await client.getEntity(channelUsername);
    await client.sendMessage(channel, { message });
};

const getChannelId = async (client, channelUsername) => {
    const channel = await client.getEntity(channelUsername);
...
const processChannelPosts = async () => {
    ...
    const sourceChannelUsername = "crypton_deploys"; // замените на имя вашего канала
    const destinationChannelId = await getChannelId(
        client,
        "your_private_channel"
    );

    const addresses = await getChannelPosts(client, sourceChannelUsername);

    if (addresses.length > 0) {
        for (const { address, link } of addresses) {
